...
class AddSingleRecordViewModel @Inject constructor(
    private val singleAddUseCase: SingleAddUseCases,
        ...
        viewModelScope.launch {
            savedStateHandle.get<String>("bookId")?.let { bookId ->
                savedStateHandle.get<String>("recordId")?.let { recordId ->
                    if (recordId != "-1") {
                        singleAddUseCase.getRecordById(
                            recordId
                        ).collectLatest {
                            state = state.copy(
        ...
        viewModelScope.launch {
            when (event) {
                is AddRecordEvent.SaveRecord -> {
                    viewModelScope.launch {
                        performCalculation()
                        singleAddUseCase.upsertRecordItem(state.copy(recordAmount = calcState.number1.toDouble()))
                            .collect { result ->
    ...
    fun onAction(event: CalcEvent) {
        when (event) {
            ...
            CalcEvent.DecimalPoint -> enterDecimal()
            CalcEvent.Clear -> calcState = CalcState()
            CalcEvent.Calculate -> performCalculation()
            CalcEvent.Delete -> performDeletion()
            CalcEvent.Percent -> performPercent()
